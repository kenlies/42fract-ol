...
void	mandelbrot(t_info *info)
{
	...
	int	iterations;
	int	i;
	double real_factor = 1.0 / (info->width * info->zoom);
	double imag_factor = 1.0 / (info->height * info->zoom);
	double width_halfed = info->width / 2;
	double height_halfed = info->height / 2;


	i = 0;
	while (i < info->all_pixels)
	{
        double x = (i % info->width - width_halfed) * real_factor + info->x_pos;
        double y = (i / info->width - height_halfed) * imag_factor + info->y_pos;
...
void	julia(t_info *info)
{
	...
	while (i < info->all_pixels)
	{
		info->z.real = (i % info->width - width_halfed) * real_factor + info->x_pos;
		info->z.imag = (i / info->width - height_halfed) * imag_factor + info->y_pos;
		iterations = iterate(info);
		color = findcolor(info, iterations);
		my_mlx_pixel_put(info, i % info->width, i / info->width, color);
		++i;
...
int	iterate(t_info *info)
{
	...
	{
		info->temp_z.real = info->z.real * info->z.real - info->z.imag * info->z.imag;
		if (info->set == 'b')
		...
		if (fabs(info->z.real + info->z.imag) > 32)
			break ;
	}
	return (iter);
}
...
int	findcolor(t_info *info, int iter)
{
	...
	clr.r = ((clr.color_1 >> 16) & 0xFF) + clr.ratio * (((clr.color_2 >> 16) & 0xFF) - ((clr.color_1 >> 16) & 0xFF));
